...
<a href="https://ubuntu.com/pro" target="_blank"><img src="https://img.shields.io/badge/Ubuntu_Pro-E95420?style=for-the-badge&logo=ubuntu&logoColor=white" alt="Ubuntu Pro"></a>
...
<a href="https://huggingface.co/Qwen/Qwen3-14B" target="_blank"><img src="https://img.shields.io/badge/Qwen_AI-604AD9?style=for-the-badge&logo=huggingface&logoColor=white" alt="Qwen AI"></a>
...
![Status](https://img.shields.io/badge/Status-Live_%26_Autonomous-22863a?style=flat-square)
![Version](https://img.shields.io/badge/SentinelNode-v1.1-blueviolet?style=flat-square)

> "Implementing Governed Cognition to secure public-facing assets through Defense-in-Depth, Real-Time Substrate Monitoring, and Autonomous Threat Analysis"
...
* **The Stack:** Hardened **Ubuntu Pro** (LTS security patching), **OpenLiteSpeed** with Native WAF, **CrowdSec** (Global Threat Intelligence), **Python 3** (`watchdog`/`inotify`), **SQLite** (Local Audit), **Dockerized PostgreSQL & Matrix/Synapse**, and the **Google Gemini API**.
* **Security Controls Implemented:**
  ...
  * **Active Host-Based Access Control:** Implemented a **"Default-Deny"** firewall policy via **UFW**, securing SSH via key-based authentication. Replaced legacy Fail2ban with **CrowdSec**, leveraging a real-time, peer-to-peer threat intelligence network that actively ingests both OpenLiteSpeed and Matrix access logs to automatically null-route malicious actors at the network layer.
  * **Real-Time File Integrity Monitoring (FIM):** Engineered **SentinelNode 1.0.1**, a custom Python daemon operating in an isolated virtual environment **outside WordPress itself** — architecturally immune to plugin-layer compromise. Transitioned from legacy cron jobs to an **event-driven `inotify` watchdog architecture**, achieving zero-latency detection of unauthorized file creation or modification. Validates core files against the live **WordPress.org upstream cryptographic API** and sensitive configs against a **`chattr +i` immutable local baseline**.
  * **Zero-Trust Substrate & Environment Gating:** Enforced strict secret decoupling by isolating all API keys and mesh tokens into a local `.env` vault. The FIM daemon, baseline configurations, and cryptographic keys are locked at the kernel level using `chattr +i` immutability, preventing sophisticated malware from tampering with the watchdog logic.
  * **NIST AU-3 Compliant Cryptographic Audit Trail:** Every scan is recorded to a **WAL-journaled SQLite** database with full forensic provenance. Every record is **HMAC-SHA256 signed**, satisfying non-repudiation requirements. A signature mismatch on daemon startup triggers an immediate process abort and alerts the administrator of database tampering.
  * **Autonomous AI Forensics (The Alethic Doctor):** Integrated the **Google Gemini 3 Flash** model into the SentinelNode pipeline. Built on Emil Uzelac's published **Governed Cognition** framework ([*"From Fragile Glue to Governed Cognition"*](https://github.com/emiluzelac/governed-cognition)), the LLM acts as a secondary reasoning engine bound by strict architectural laws: an epistemic confidence constraint (`>= 0.90`), a deterministic scope gate, and a **mass-infection circuit breaker** (>5 simultaneous anomalies bypasses AI entirely). This enables real-time forensic analysis of obfuscated zero-day payloads without relying on traditional malware signatures.
  * **Sovereign SecOps Alerting Mesh:** Engineered a complete migration from public APIs (Telegram) to a **Self-Hosted Matrix/Synapse** server deployed via Docker and backed by **PostgreSQL 16**. This guarantees end-to-end encrypted, zero-latency forensic alerts pushed directly to mobile (Element X) while maintaining absolute data sovereignty. 
  * **Automated Remediation (The Quarantine Vault):** Replaced static alerting with an active "Soft Healing" pipeline. Upon governed AI confirmation of a malicious payload, SentinelNode surgically neutralizes the threat by timestamping and moving the infected file to a non-executable, isolated **Quarantine Vault**, triggering a 30-day automated cron retention purge to maintain disk hygiene.
  * **Roadmap — Predictive Simulation & Adaptive Banning:** Integrating a "Simulator Worker" to syntax-check (`php -l`) AI-proposed code surgically removed from live files before committing changes, guaranteeing zero application downtime. Furthermore, utilizing the framework's `AdaptiveWorker` to autonomously propose permanent CrowdSec network bans for IPs that repeatedly trigger the `inotify` watchdog with failed upload vectors.
